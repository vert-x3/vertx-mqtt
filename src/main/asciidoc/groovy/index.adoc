...
def client = MqttClient.create(vertx, options)
...
Another simple way to connect without using the `link:../../apidocs/io/vertx/mqtt/MqttClientOptions.html[MqttClientOptions]` is to use the
`link:../../apidocs/io/vertx/mqtt/MqttClient.html#connect-int-java.lang.String-io.vertx.core.Handler-[connect]` passing the port and the host directly.
