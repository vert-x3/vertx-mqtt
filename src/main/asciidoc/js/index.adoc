...
Another simple way to connect without using the `link:../dataobjects.html#MqttClientOptions[MqttClientOptions]` is to use the
`link:../../jsdoc/module-vertx-mqtt-server-js_mqtt_client-MqttClient.html#connect[connect]` passing the port and the host directly.
