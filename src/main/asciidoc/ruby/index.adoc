...
----
compile io.vertx:vertx-mqtt-server:3.5.0-SNAPSHOT
----
...
+
You could provide handler by calling `link:../../yardoc/VertxMqttServer/MqttClient.html#publish_complete_handler-instance_method[publishCompleteHandler]`. The handler will be called each time publish is completed.
This one is pretty useful because you could see the packetId of just received PUBACK or PUBCOMP packet.
[source,ruby]
----
require 'vertx/buffer'
client.publish_complete_handler() { |id|
...
[source,ruby]
----
client.subscribe_complete_handler() { |mqttSubAckMessage|
...
[source,ruby]
----
client.unsubscribe_complete_handler() { |id|
