...
public class Mqtt5ClientSubscribeTest {
  ...
  public void subscribeWithSubscriptionIdentifier(TestContext ctx) {
    ...
    server.endpointHandler(endpoint -> {
      endpoint.subscribeHandler(subscribe -> {
        MqttProperties.MqttProperty<?> prop = subscribe.properties().getProperty(MqttProperties.MqttPropertyType.SUBSCRIPTION_IDENTIFIER.value());
        ...
        endpoint.subscribeAcknowledge(subscribe.messageId(),
            java.util.Arrays.asList(MqttSubAckReasonCode.qosGranted(subscribe.topicSubscriptions().get(0).qualityOfService())),
    ...
    startServer(ctx, () -> {
      ...
      client.connect(server.actualPort(), "localhost")
          .onComplete(ctx.asyncAssertSuccess(ack -> {
            ...
            props.add(new MqttProperties.IntegerProperty(MqttProperties.MqttPropertyType.SUBSCRIPTION_IDENTIFIER.value(), SUBSCRIPTION_IDENTIFIER));
            client.subscribe(java.util.Collections.singletonMap(MQTT_TOPIC, 1), props);
          }));
    });
  ...
  public void subscribeAckGrantedQos1(TestContext ctx) {
    ...
    startServer(ctx, () -> {
      ...
      client.connect(server.actualPort(), "localhost")
          .onComplete(ctx.asyncAssertSuccess(ack -> client.subscribe(java.util.Collections.singletonMap(MQTT_TOPIC, 1))));
    });
  ...
  public void subscribeAckError(TestContext ctx) {
    ...
    server.endpointHandler(endpoint -> {
      endpoint.subscribeHandler(subscribe -> endpoint.subscribeAcknowledge(subscribe.messageId(),
          java.util.Arrays.asList(MqttSubAckReasonCode.UNSPECIFIED_ERROR),
          MqttProperties.NO_PROPERTIES));
      endpoint.accept(false);
    });
    ...
    startServer(ctx, () -> {
      ...
      client.connect(server.actualPort(), "localhost")
          .onComplete(ctx.asyncAssertSuccess(ack -> client.subscribe(java.util.Collections.singletonMap("/topic", MqttQoS.AT_LEAST_ONCE.value()))));
    });
  ...
  public void subscribeAckWithReasonStringProperty(TestContext ctx) {
    ...
    server.endpointHandler(endpoint -> {
      endpoint.subscribeHandler(subscribe -> {
        ...
        subackProps.add(new MqttProperties.StringProperty(MqttProperties.MqttPropertyType.REASON_STRING.value(), reasonString));
        endpoint.subscribeAcknowledge(subscribe.messageId(), java.util.Arrays.asList(MqttSubAckReasonCode.GRANTED_QOS0), subackProps);
      });
      endpoint.accept(false);
    });
    ...
    startServer(ctx, () -> {
      ...
      client.subscribeCompletionHandler((MqttSubAckMessage ack) -> {
        MqttProperties.MqttProperty<?> prop = ack.properties().getProperty(MqttProperties.MqttPropertyType.REASON_STRING.value());
      ...
      client.connect(server.actualPort(), "localhost")
          .onComplete(ctx.asyncAssertSuccess(ack -> client.subscribe(java.util.Collections.singletonMap(MQTT_TOPIC, 0))));
    });
  ...
  public void subscribeWithWildcardRejectedWhenServerDisablesIt(TestContext ctx) {
    ...
    server.endpointHandler(endpoint -> {
      MqttProperties connAckProps = new MqttProperties();
      connAckProps.add(new MqttProperties.IntegerProperty(
        MqttProperties.MqttPropertyType.WILDCARD_SUBSCRIPTION_AVAILABLE.value(), 0));
      endpoint.accept(false, connAckProps);
    });
    ...
    startServer(ctx, () -> {
      ...
      client.connect(server.actualPort(), "localhost")
        .onComplete(ctx.asyncAssertSuccess(ack -> {
          ...
          client.subscribe(java.util.Collections.singletonMap("/wildcard/+/topic", 1))
            .onComplete(ctx.asyncAssertFailure(err -> {
              ctx.assertTrue(err instanceof MqttException, "Expected MqttException");
  ...
  public void subscribeWithSharedRejectedWhenServerDisablesIt(TestContext ctx) {
    ...
    server.endpointHandler(endpoint -> {
      MqttProperties connAckProps = new MqttProperties();
      connAckProps.add(new MqttProperties.IntegerProperty(
        MqttProperties.MqttPropertyType.SHARED_SUBSCRIPTION_AVAILABLE.value(), 0));
      endpoint.accept(false, connAckProps);
    });
    ...
    startServer(ctx, () -> {
      ...
      client.connect(server.actualPort(), "localhost")
        .onComplete(ctx.asyncAssertSuccess(ack -> {
          ...
          client.subscribe(java.util.Collections.singletonMap("$share/group/topic", 1))
            .onComplete(ctx.asyncAssertFailure(err -> {
              ctx.assertTrue(err instanceof MqttException, "Expected MqttException");
    ...
    server.endpointHandler(endpoint -> {
      endpoint.subscribeHandler(sub ->
        endpoint.subscribeAcknowledge(sub.messageId(),
          java.util.Arrays.asList(MqttSubAckReasonCode.GRANTED_QOS0), MqttProperties.NO_PROPERTIES));
    ...
    startServer(ctx, () -> {
      ...
      client.connect(server.actualPort(), "localhost")
        .onComplete(ctx.asyncAssertSuccess(ack -> {
          ...
          client.subscribe(java.util.Collections.singletonMap("/wildcard/#", 0))
