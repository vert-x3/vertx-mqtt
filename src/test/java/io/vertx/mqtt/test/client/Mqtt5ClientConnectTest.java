...
public class Mqtt5ClientConnectTest {
  ...
  public void connectWithSessionExpireInterval(TestContext ctx) {
    ...
    server.endpointHandler(endpoint -> {
      MqttProperties.MqttProperty<?> prop = endpoint.connectProperties().getProperty(MqttProperties.MqttPropertyType.SESSION_EXPIRY_INTERVAL.value());
  ...
  public void connectWithReceiveMaximum(TestContext ctx) {
    ...
    server.endpointHandler(endpoint -> {
      MqttProperties.MqttProperty<?> prop = endpoint.connectProperties().getProperty(MqttProperties.MqttPropertyType.RECEIVE_MAXIMUM.value());
      ctx.assertNotNull(prop);
  ...
  public void connectWithMaximumPacketSize(TestContext ctx) {
    ...
    server.endpointHandler(endpoint -> {
      MqttProperties.MqttProperty<?> prop = endpoint.connectProperties().getProperty(MqttProperties.MqttPropertyType.MAXIMUM_PACKET_SIZE.value());
      ctx.assertNotNull(prop);
      ctx.assertEquals(expected, Integer.toUnsignedLong((Integer) prop.value()));
      endpoint.accept(false);
      serverLatch.complete();
  ...
  public void connectWithTopicAliasMaximum(TestContext ctx) {
    ...
    server.endpointHandler(endpoint -> {
      MqttProperties.MqttProperty<?> prop = endpoint.connectProperties().getProperty(MqttProperties.MqttPropertyType.TOPIC_ALIAS_MAXIMUM.value());
      ctx.assertNotNull(prop);
      ctx.assertEquals(expected.longValue(), Integer.toUnsignedLong((Integer) prop.value()));
      endpoint.accept(false);
      serverLatch.complete();
  ...
  public void connectWithAllProperties(TestContext ctx) {
    ...
    server.endpointHandler(endpoint -> {
      ...
      ctx.assertNotNull(props.getProperty(MqttProperties.MqttPropertyType.SESSION_EXPIRY_INTERVAL.value()));
      ctx.assertNotNull(props.getProperty(MqttProperties.MqttPropertyType.RECEIVE_MAXIMUM.value()));
      ctx.assertNotNull(props.getProperty(MqttProperties.MqttPropertyType.MAXIMUM_PACKET_SIZE.value()));
      ctx.assertNotNull(props.getProperty(MqttProperties.MqttPropertyType.TOPIC_ALIAS_MAXIMUM.value()));
      ctx.assertEquals(60l, Integer.toUnsignedLong((Integer) props.getProperty(MqttProperties.MqttPropertyType.SESSION_EXPIRY_INTERVAL.value()).value()));
      ctx.assertEquals(20l, Integer.toUnsignedLong((Integer) props.getProperty(MqttProperties.MqttPropertyType.RECEIVE_MAXIMUM.value()).value()));
      ctx.assertEquals(32768l, Integer.toUnsignedLong((Integer) props.getProperty(MqttProperties.MqttPropertyType.MAXIMUM_PACKET_SIZE.value()).value()));
      ctx.assertEquals(3l, Integer.toUnsignedLong((Integer) props.getProperty(MqttProperties.MqttPropertyType.TOPIC_ALIAS_MAXIMUM.value()).value()));
      endpoint.accept(false);
      serverLatch.complete();
    });
  ...
  public void connectWithAuthenticationMethod(TestContext ctx) {
    ...
    server.endpointHandler(endpoint -> {
      MqttProperties.MqttProperty<?> prop = endpoint.connectProperties().getProperty(MqttProperties.MqttPropertyType.AUTHENTICATION_METHOD.value());
      ctx.assertNotNull(prop);
  ...
  public void connectWithAuthenticationMethodAndData(TestContext ctx) {
    ...
    server.endpointHandler(endpoint -> {
      MqttProperties props = endpoint.connectProperties();
      MqttProperties.MqttProperty<?> methodProp = props.getProperty(MqttProperties.MqttPropertyType.AUTHENTICATION_METHOD.value());
      MqttProperties.MqttProperty<?> dataProp   = props.getProperty(MqttProperties.MqttPropertyType.AUTHENTICATION_DATA.value());
  ...
  public void connackResponseInformation(TestContext ctx) {
    ...
    server.endpointHandler(endpoint -> {
      ...
      MqttProperties.MqttProperty<?> reqProp =
        endpoint.connectProperties().getProperty(MqttProperties.MqttPropertyType.REQUEST_RESPONSE_INFORMATION.value());
      ...
      connAckProps.add(new MqttProperties.StringProperty(MqttProperties.MqttPropertyType.RESPONSE_INFORMATION.value(), expected));
  ...
  public void connectWithUserProperties(TestContext ctx) {
    ...
    server.endpointHandler(endpoint -> {
      MqttProperties.MqttProperty<?> prop = endpoint.connectProperties().getProperty(MqttProperties.MqttPropertyType.USER_PROPERTY.value());
    ...
    startServer(ctx, () -> {
      ...
      MqttClient.create(vertx, options)
          .connect(server.actualPort(), "localhost", null, java.util.Collections.singletonMap("k1", "v1"));
    });
