...
public class Mqtt5ClientUnsubscribeTest {
  ...
  public void unsubscribeSuccessReasonCode(TestContext ctx) {
    ...
    server.endpointHandler(endpoint -> {
      ...
      endpoint.unsubscribeHandler(unsubscribe -> endpoint.unsubscribeAcknowledge(unsubscribe.messageId(),
          java.util.Arrays.asList(MqttUnsubAckReasonCode.SUCCESS), MqttProperties.NO_PROPERTIES));
    ...
    startServer(ctx, () -> {
      ...
      client.connect(server.actualPort(), "localhost")
          .onComplete(ctx.asyncAssertSuccess(connAck -> client.subscribe(java.util.Collections.singletonMap(MQTT_TOPIC, 0))
              .onComplete(ctx.asyncAssertSuccess(subId -> client.unsubscribe(java.util.Arrays.asList(MQTT_TOPIC))))));
    });
  ...
  public void unsubscribeNotSubscribedReasonCode(TestContext ctx) {
    ...
    server.endpointHandler(endpoint -> {
      endpoint.unsubscribeHandler(unsubscribe -> endpoint.unsubscribeAcknowledge(unsubscribe.messageId(),
          java.util.Arrays.asList(MqttUnsubAckReasonCode.NO_SUBSCRIPTION_EXISTED), MqttProperties.NO_PROPERTIES));
    ...
    startServer(ctx, () -> {
      ...
      client.connect(server.actualPort(), "localhost")
          .onComplete(ctx.asyncAssertSuccess(connAck -> client.unsubscribe(java.util.Arrays.asList(MQTT_TOPIC))));
    });
    ...
    startServer(ctx, () -> {
      ...
      client.connect(server.actualPort(), "localhost")
          .onComplete(ctx.asyncAssertSuccess(connAck -> {
            ...
            client.unsubscribe(java.util.Arrays.asList(MQTT_TOPIC), props);
          }));
    });
  ...
  public void unsubscribeAckWithReasonStringProperty(TestContext ctx) {
    ...
    server.endpointHandler(endpoint -> {
      endpoint.unsubscribeHandler(unsubscribe -> {
        ...
        endpoint.unsubscribeAcknowledge(unsubscribe.messageId(),
            java.util.Arrays.asList(MqttUnsubAckReasonCode.NO_SUBSCRIPTION_EXISTED), subackProps);
      });
      endpoint.accept(false);
    });
